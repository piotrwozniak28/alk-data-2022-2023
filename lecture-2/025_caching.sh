...
gsutil cp "${COMMON_RESOURCES_DIR}"/"${OBJECT_NAME}" gs://"${BUCKET_NAME_2}"
gsutil ls gs://"${BUCKET_NAME_2}"/"${OBJECT_NAME}"


gsutil setmeta -h "cache-control:no-store" gs://"${BUCKET_NAME_2}"/"${OBJECT_NAME}"
# gsutil setmeta -h "cache-control:public, max-age=3600" gs://"${BUCKET_NAME_2}"/"${OBJECT_NAME}"
